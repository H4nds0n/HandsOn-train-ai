...
# Load the TensorFlow Lite model
interpreter = tf.lite.Interpreter(model_path="/Users/floriankainberger/ai/handsOn/HandsOn-train-ai/mobile/models/model30/asl_model.tflite")
interpreter.allocate_tensors()
...
# # Load the TensorFlow Lite model
# # interpreter = tf.lite.Interpreter(model_path="models/model21/out/asl_model.tflite")
# # interpreter.allocate_tensors()

# #20 is not good (overfitted probably)
# #21 is alright, doesn't recognize As tho
# #22 is shit (only As)
# #23 is shit (only Bs) overfitted
# #24 is shit (only Cs) overfitted
# #29_0 feeling good but 100%?? ==> probably because high lr (0.83)
# model = load_model("models/model30/asl_model_keras.keras", compile=False)

# # Load the labels
# class_names = open("models/model28/labels.txt", "r").readlines()
